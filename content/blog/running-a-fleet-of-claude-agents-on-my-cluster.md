...
Every interface between those pieces is protobuf — one package, generated by `buf` into `connect-go` handlers for Go, `protoc-gen-es` clients for the browser, and types-only bindings for the worker, which never speaks gRPC at all. The transports split by caller: in-cluster traffic is plain gRPC over HTTP/2, and the console talks ConnectRPC over HTTP/1.1, which replaced a REST API plus an SSE endpoint. `connect-go`'s handler will also decode gRPC and gRPC-Web framing on that same path, but the server it is mounted on has neither h2c nor TLS/ALPN, so that compatibility is nominal rather than a working gRPC endpoint — the decision record says exactly that, which is the sort of honesty that makes the rest of a document trustworthy.
...
flowchart TB
    ...
    side -->|"one gRPC connection, out only"| core
...
There is an agent whose scope is the cluster itself. It started as a standing service with its own permanent cluster permissions, and within four days I deleted the standing service and made it an ordinary session — same pods, same permission prompts, same dashboard, no special case. The only privileged thing left is a tiny component with a single RPC — `Exec`, whose request field is `repeated string args`: an array, never a command string, so shell metacharacters are inert by construction rather than filtered out by a regex somebody has to keep right. It runs on behalf of a pod that holds no credentials at all. Read calls are checked against a verb allowlist, with `rollout` deliberately excluded because `rollout restart` mutates, and with impersonation and credential flags — `--as`, `--kubeconfig`, `--token` — denied outright. Mutations are a dumb pipe, because a human already approved that exact argument list through the permission prompt, and a second allowlist there would only constrain what a human is permitted to decide.
